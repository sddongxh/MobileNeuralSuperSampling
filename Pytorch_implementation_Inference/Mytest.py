...
if model == "Q":
    ...
    is_half = False
    checkpoint = r"./checkpoint/checkpoint-epochQ.pth"
elif model== "B":
    from model.MNSS_B import MNSS
    is_half = True
    checkpoint = r"./checkpoint/checkpoint-epochB.pth"
elif model== "P":
    from model.MNSS_P import MNSS
    is_half = True
    checkpoint = r"./checkpoint/checkpoint-epochP.pth"
...
def main(config):
    ...
    with torch.no_grad():
        ...
        if is_half:
            previous_x_RGBD = previous_x_RGBD.half()
        ...
        for batch_idx, (x_view, x_depth, x_flow, target, image_name) in enumerate(tqdm_bar):
            ...
            for i in range(pred.shape[0]):
                ...
                img = pred[i]
                save_name = os.path.join(config['save_dir'], checkpoint[-5:-4])
